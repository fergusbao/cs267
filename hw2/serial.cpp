...
int main(int argc, char **argv) {
    ...
    auto particles = real_buffer.data();
#endif
    set_size(n);
    ...
    for (int step = 0; step < NSTEPS; step++) {
        ...
        for (int i = 0; i < n; i++)
            move(particles[i]);
#else
        auto grids = r267::do_grid(real_buffer);
        r267::compute_forces(grids, real_buffer, &dmin, &davg, &navg);
        r267::move_them(real_buffer);
#endif
    ...
    if (find_option(argc, argv, "-no") == -1) {
        ...
        //  simulation where particles don't interact correctly will be less
